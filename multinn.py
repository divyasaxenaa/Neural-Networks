...
class MultiNN(object):
    ...
    def add_layer(self, num_nodes, activation_function):
        """
         ...
         :return: None
         """
        layer = [self.layer_no,num_nodes, activation_function]
        self.total_layers.insert(self.layer_no, layer)
        weight = tf.ones([self.input_dimension, num_nodes])
        self.weights.insert(self.layer_no, weight)
        bias = tf.ones([num_nodes])
        self.biases.insert(self.layer_no, bias)
        self.act_func.insert(self.layer_no,activation_function)
    ...
    def predict(self, X):
        ...
        """
        layer = 0
        for layer_number in range(self.layer_no):
            X = self.act_func[layer_number](tf.matmul(X, self.weights[layer])+self.biases[layer])
            self.lay_output.insert(self.layer_no, X)
            layer = layer + 1
        return X
    ...
    def train(self, X_train, y_train, batch_size, num_epochs, alpha=0.8, regularization_coeff=1e-6):
        ...
        for epoch in range(num_epochs):
            for stp, (x, y) in enumerate(tf.data.Dataset.from_tensor_slices((X_train, y_train)).batch(batch_size)):
                with tf.GradientTape(persistent=True) as tape:
                    error = self.cross_entropy_loss(y, self.predict(x))
                    for layerno in range(len(self.weights)):
                        dloss_weight, dloss_bias = tape.gradient(error, [self.weights[layerno], self.biases[layerno]])
                        self.weights[layerno].assign_sub(alpha * dloss_weight)
                        self.biases[layerno].assign_sub(alpha * dloss_bias)
    ...
    def calculate_percent_error(self, X, y):
        ...
        for i in range(y.shape[0]):
            if predicted[i] == y[i]:
                count = count + 1
        percent_error = (y.shape[0] - count) / y.shape[0]
        return percent_error
    ...
    def calculate_confusion_matrix(self, X, y):
        ...
        """
        cm = np.zeros((self.input_dimension, self.input_dimension))
        predict_value = np.argmax(self.predict(X), axis=1)
        for i in range(y.shape[0]):
            cm[predict_value[i]][y[i]] = cm[predict_value[i]][predict_value[i]] + 1
        return cm.T
